...
def start(artigo):  # Metodo que recebe uma dict, usado para nomear as pastas
    ...
    def diretorios():  # Metodo qye criara os diretorios
        ...
        if so == 'Linux':
            ...
            os.makedirs(dir + 'Arquivos/' + artigo['termo'] + '/',
                        exist_ok=True)  # Crio a pasta Arquivos e outra com o titulo da busca, se existir nao me retorna nenhum exeçao
            dir = os.path.expanduser('~/Documentos/Arquivos/' + artigo[
                'termo'] + '/')  # Pego o caminho completo para ser retornado ao robo de texto
            return dir
        elif so == 'Windows':
            print("...")
